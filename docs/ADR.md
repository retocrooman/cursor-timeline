...
## ADR-023: v0.2 コスト突合は Team usage CSV + 時刻窓

**Status:** Accepted（PoC 済み）

**Context:** ローカル DB は user プロンプトと時刻を持つが token / $ は空。Cursor Dashboard の Team usage CSV はリクエスト単位の $ と時刻を持つが session ID・プロンプト本文はない（ADR-003）。

**Decision:**
1. **データ源:** Cursor Team の usage events **CSV 手動 export**（Toolbar「Import CSV…」）。非公式 API は PoC 後に必要なら検討。
2. **突合キー:** プロンプト `timestamp` と usage `Date` の **時刻窓**（session ID は使わない）。
3. **窓（PoC 確定）:** `pre=30s`, `post=300s`, `rangeSlack=300s`。窓拡大でセッション突合率は横ばい（~84%）、`post=300s` は orphan 減・コスト割当率向上のため採用。
4. **割当:** usage 1 行 → 窓内の **直前プロンプト** に greedy 割当（1 event = 1 prompt）。
5. **表示（v0.2 UI）:**
   - **exact:** 窓内 events の Cost 合算 → Inspector 行に `$`
   - **range:** 拡張窓内の min–max Cost → `$0.02–0.05?`
   - **unmatched:** `$` 非表示
6. **セッション指標:** 「プロンプト 1 件以上あるセッション」のうち **1 件でも exact があれば突合成功**。

**PoC 結果（2026-06、1 ユーザー・1 週間 CSV）:**
...
| セッション exact 突合 | **83.9%** |
| プロンプト exact 突合 | **64.8%** |
| プロンプト exact+range | **92.0%** |

**Consequences:**
- プロンプト単位は 1 prompt → 複数 usage 行（agent 返答等）が前提。セッション単位の方が UX 上の突合率は高い。
- orphan usage（~54% 行）は bubble のない API 呼び出し。v0.2 では集計のみ、非表示。
...
| 自動 CSV 取得 | 非公式 API or 定期 export |
