...
**入力:**
- ローカル `PromptEvent`（bubble 由来、時刻・本文・model）
- Team usage CSV 行（`Date`, `Model`, `Cost`, token 列）

**出力:** `PromptCostEstimate` — プロンプト行ごとの `$` またはレンジ

**フロー:**
```
Import CSV → UsageRecord[]
     +
TimelineStore prompts → UsageReconciler.reconcile(config)
     → Inspector に cost 列を追加
```
...
| 状態 | 条件 | Inspector 表示 |
|------|------|----------------|
| exact | post 窓内に usage ≥1 | `$0.18`（N requests） |
| range | 拡張窓のみ | `$0.02–0.05?` |
| unmatched | 近傍なし | （非表示） |

**実装:** `Core/Reconcile/` — `UsageCSVReader`, `UsageReconciler`, `ReconciliationPoCRunner`  
**Verify:** `./scripts/reconcile-poc.sh [csv]`

---

## 9. プロジェクト構成（SwiftPM — Clippo 流儀）
